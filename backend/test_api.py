...
class TestReceivablesProxy:
    ...
    def test_commercial_billing_candidates_forwards_read_only_preview(
        self, client, auth, monkeypatch
    ):
        import time_tracker_api as api
        ...
        preview = {
            ...
            "candidates": [
                {
                    "candidateKey": "commercial-billing:acme:2026-03",
                    "sourceFingerprint": "candidate-fingerprint",
                    "blockers": [],
                }
            ],
            "summary": {"candidateCount": 1, "blockedCandidateCount": 0},
        }
        calls = []

        def fake_request(method, url, **kwargs):
            calls.append((method, url, kwargs))
            return _AtlasResponse(preview)
        ...
        response = client.get(
            "/api/admin/receivables/commercial-billing-candidates",
        ...
        method, url, kwargs = calls[0]
        assert method == "GET"
        assert url == "https://atlas.test/api/v1/receivables/commercial-billing-candidates"
        assert kwargs["headers"]["Authorization"] == (
            f"Bearer {GENERATED_RECEIVABLES_TOKEN}"
        )
        assert kwargs["headers"]["X-EOM-Actor"] == "Juan Canfield"
        assert "Idempotency-Key" not in kwargs["headers"]
        assert kwargs["json"] is None
        assert kwargs["params"] == {"billing_period": "2026-03"}

    def test_commercial_billing_candidates_requires_current_admin_session_before_an_upstream_read(
        self, client, emp_auth, monkeypatch
    ):
        import time_tracker_api as api
        ...
        unauthenticated = client.get(
            "/api/admin/receivables/commercial-billing-candidates?billing_period=2026-03",
        )
        employee = client.get(
            "/api/admin/receivables/commercial-billing-candidates?billing_period=2026-03",
            headers=emp_auth,
    ...
    @pytest.mark.parametrize(
        ...
        [
            "/api/admin/receivables/commercial-billing-candidates",
            "/api/admin/receivables/commercial-billing-candidates?billing_period=2026-3",
            "/api/admin/receivables/commercial-billing-candidates?billing_period=2026-00",
            "/api/admin/receivables/commercial-billing-candidates?billing_period=2026-13",
            "/api/admin/receivables/commercial-billing-candidates?billing_period=not-a-month",
        ],
    )
    def test_commercial_billing_candidates_rejects_invalid_period_before_atlas(
        self, client, auth, monkeypatch, path
    ):
        import time_tracker_api as api
    ...
    def test_commercial_billing_candidates_retry_after_upstream_outage_is_a_fresh_read(
        self, client, auth, monkeypatch
    ):
        import time_tracker_api as api
            ...
            return _AtlasResponse(
                {
                    "billingPeriod": "2026-03",
                    "candidates": [],
                    "summary": {"candidateCount": 0, "blockedCandidateCount": 0},
                }
            )
        ...
        assert failed.headers["retry-after"] == "5"
        assert recovered.status_code == 200
        assert recovered.json()["billingPeriod"] == "2026-03"
        assert [call[0] for call in calls] == ["GET", "GET"]
        assert calls[0][1] == calls[1][1]
    ...
    def test_commercial_billing_approval_pdf_gmail_draft_reconcile_and_missing_draft_replacement_proxy_without_local_state(
        ...
        def fake_request(method, url, **kwargs):
            ...
            if url.endswith("/approvals"):
                return _AtlasResponse(approval_result, status_code=201)
        ...
        approval = client.post(
            f"/api/admin/receivables/commercial-billing-runs/{billing_run_id}/approvals",
            headers={**auth, "Idempotency-Key": "approve-acme-2026-03"},
            json={
                "candidate_key": "candidate:acme:2026-03",
                "expected_source_fingerprint": source_fingerprint,
                "expected_review_fingerprint": review_fingerprint,
        ...
        pdf = client.post(
            ...
            headers={**auth, "Idempotency-Key": "pdf-acme-2026-03"},
        )
        draft = client.post(
            f"/api/admin/receivables/commercial-billing-approvals/{approval_id}/gmail-draft",
            headers={**auth, "Idempotency-Key": "draft-acme-2026-03"},
        )
        ...
        assert calls[2][1].endswith(
            f"/commercial-billing-approvals/{approval_id}/gmail-draft"
        )
        ...
        assert audits == [
            (
                ...
                True,
                "Commercial billing candidate approval accepted by Atlas for Juan Canfield",
            ),
            (
                "RECEIVABLES_COMMERCIAL_BILLING_INVOICE_PDF_CREATE",
                True,
                "Commercial billing invoice PDF accepted by Atlas for Juan Canfield",
            ),
            (
    ...
    def test_commercial_billing_gmail_draft_survives_local_audit_failure(
        self, client, auth, monkeypatch
    ):
        import time_tracker_api as api
    ...
    def test_commercial_billing_approval_preserves_atlas_stale_conflict_without_local_state(
        self, client, auth, monkeypatch
    ):
        import time_tracker_api as api
        ...
        def stale_provider(method, url, **kwargs):
            ...
            return _AtlasResponse(
                {
                    "detail": {
                        "code": "commercial_billing_approval_stale",
                        "message": "Candidate source evidence changed; regenerate it",
                    }
                },
                status_code=409,
        ...
        response = client.post(
            f"/api/admin/receivables/commercial-billing-runs/{billing_run_id}/approvals",
            headers={**auth, "Idempotency-Key": "stale-approval-2026-03"},
            json={
                "candidate_key": "candidate:acme:2026-03",
                "expected_source_fingerprint": "a" * 64,
            },
    ...
    def test_commercial_billing_gmail_commands_reject_stale_sessions_and_invalid_input_before_atlas(
        self, client, auth, emp_auth, monkeypatch
    ):
        import time_tracker_api as api
        ...
        unauthenticated = client.post(
            approval_path,
            headers={"Idempotency-Key": "unauthenticated-approval"},
            json={
                "candidate_key": "candidate:acme:2026-03",
                "expected_source_fingerprint": "a" * 64,
            },
        ...
        employee = client.post(
            approval_path,
            headers={**emp_auth, "Idempotency-Key": "employee-approval"},
            json={
                "candidate_key": "candidate:acme:2026-03",
                "expected_source_fingerprint": "a" * 64,
            },
        ...
        expired = client.post(
            ...
            headers={
                "Authorization": f"Bearer {expired_token}",
                "Idempotency-Key": "expired-approval",
            },
            json={
                "candidate_key": "candidate:acme:2026-03",
                "expected_source_fingerprint": "a" * 64,
        ...
        invalid_run = client.post(
            "/api/admin/receivables/commercial-billing-runs/not-a-uuid/approvals",
            headers={**auth, "Idempotency-Key": "invalid-run"},
            json={
                "candidate_key": "candidate:acme:2026-03",
                "expected_source_fingerprint": "a" * 64,
            },
        ...
        stale_fingerprint = client.post(
            approval_path,
            headers={**auth, "Idempotency-Key": "stale-fingerprint"},
            json={
                "candidate_key": "candidate:acme:2026-03",
    ...
    def test_commercial_billing_gmail_draft_retry_reuses_provider_key_after_ambiguous_transport_failure(
        ...
        headers = {**auth, "Idempotency-Key": "gmail-draft-retry"}
        ...
        assert calls[0][2]["headers"]["Idempotency-Key"] == calls[1][2][
            "headers"
        ]["Idempotency-Key"] == "gmail-draft-retry"
        assert calls[0][2]["json"] is calls[1][2]["json"] is None
        assert db.query_one(
            "SELECT COUNT(*) AS n FROM receivables_operation_attempts"
        )["n"] == 0
    ...
    def test_residential_payment_receipt_delivery_proxies_durable_status_without_local_state(
        self, client, auth, monkeypatch
    ):
        import time_tracker_api as api
        ...
        provider_result = {
            ...
            "receipt_delivery": {
                ...
                "skip_reason": None,
                "sent_at": None,
                "recovery_required_at": "2026-08-15T20:00:00+00:00",
            },
            "operation": {
                "state": "completed",
                "outcome": "recovery_required",
                "requested_at": "2026-08-15T20:00:00+00:00",
                "completed_at": "2026-08-15T20:00:01+00:00",
            },
            "replayed": False,
            "reused": False,
        }
        ...
        response = client.post(
            f"/api/admin/receivables/payments/{payment_id}/receipt-delivery",
            headers={**auth, "Idempotency-Key": "residential-receipt-1001"},
        )
        ...
        assert url == (
            ...
            f"{payment_id}/receipt-delivery"
        )
        assert kwargs["headers"]["Authorization"] == (
            f"Bearer {GENERATED_RECEIVABLES_TOKEN}"
        )
        assert kwargs["headers"]["X-EOM-Actor"] == "Juan Canfield"
        assert kwargs["headers"]["Idempotency-Key"] == "residential-receipt-1001"
        assert kwargs["json"] is None
        assert kwargs["params"] is None
        assert db.query_one(
            "SELECT COUNT(*) AS n FROM receivables_operation_attempts"
        ...
        assert audits == [
            (
                ...
                "for Juan Canfield; payment unchanged",
            )
        ]

    def test_residential_payment_receipt_delivery_rejects_stale_or_invalid_requests_before_atlas(
        self, client, auth, emp_auth, monkeypatch
    ):
        import time_tracker_api as api

        payment_id = "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"
        path = f"/api/admin/receivables/payments/{payment_id}/receipt-delivery"
        calls = []
        monkeypatch.setattr(
            api.requests,
            "request",
        ...
        unauthenticated = client.post(
            path, headers={"Idempotency-Key": "receipt-unauthenticated"}
        )
        employee = client.post(
            path,
            headers={**emp_auth, "Idempotency-Key": "receipt-employee"},
        )
        expired = client.post(
            path,
            headers={
                "Authorization": f"Bearer {expired_token}",
                "Idempotency-Key": "receipt-expired",
            },
        )
        missing_key = client.post(path, headers=auth)
        invalid_payment = client.post(
            "/api/admin/receivables/payments/not-a-uuid/receipt-delivery",
            headers={**auth, "Idempotency-Key": "receipt-invalid-payment"},
        )
    ...
    def test_residential_payment_receipt_delivery_retry_preserves_provider_key_after_ambiguous_transport_failure(
        ...
        provider_result = {
            ...
            "receipt_delivery": {
                ...
                "status": "sent",
                "skip_reason": None,
                "sent_at": "2026-08-15T20:00:01+00:00",
                "recovery_required_at": None,
            },
            "operation": {
                "state": "completed",
        ...
        def flaky_request(method, url, **kwargs):
            calls.append((method, url, kwargs))
            if len(calls) == 1:
                raise api.requests.ConnectionError("response lost after dispatch")
            return _AtlasResponse(provider_result)
        ...
        path = f"/api/admin/receivables/payments/{payment_id}/receipt-delivery"
        headers = {**auth, "Idempotency-Key": "residential-receipt-retry"}
        ...
        assert calls[0][2]["headers"]["Idempotency-Key"] == calls[1][2][
            "headers"
        ]["Idempotency-Key"] == "residential-receipt-retry"
        assert calls[0][2]["json"] is calls[1][2]["json"] is None
        assert db.query_one(
            "SELECT COUNT(*) AS n FROM receivables_operation_attempts"
        ) == {"n": 0}
