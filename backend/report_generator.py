...
class MonthlyReportGenerator:
    ...
    def generate_monthly_report(self, employee_data: Dict[str, Any],
                              month: int, year: int,
        ...
        try:
            ...
            story.append(Paragraph(f"MONTHLY HOURS REPORT", title_style))
            ...
            summary_table_data = [
                ...
                ["Gross Profit", f"${(summary_data.get('total_revenue', 0.0) - summary_data.get('total_labor_cost', 0.0)):,.2f}"],
            ...
            employee_table = Table(employee_table_data, colWidths=[2.2 * inch, 1 * inch, 1.2 * inch, 1.2 * inch, 1 * inch])
    ...
    def _calculate_daily_breakdown(self, employee_data: Dict[str, Any],
                                 month: int, year: int) -> List[Dict[str, Any]]:
        """Calculate daily breakdown of hours"""
        daily_data = {}
        ...
        for emp in employee_data.get('employees', []):
            for shift in emp.get('shifts', []):
                ...
                if date in daily_data:
                    daily_data[date]['total_hours'] += shift.get('hours', 0)
        ...
        for date_str in sorted(daily_data.keys()):
            ...
            avg_hours = day_data['total_hours'] / employee_count if employee_count > 0 else 0
