...
def test_broad_residential_selection_cannot_hide_legacy_commercial_winner(
    client,
    monkeypatch,
):
    _employee_id, employee_auth = _create_employee(
        client,
        "broad residential commercial winner",
    )
    monkeypatch.setattr(api, "_active_home_base_config", lambda *, cur=None: None)
    monkeypatch.setattr(api, "GEOFENCE_SITE_RESOLUTION_ENABLED", True)
    monkeypatch.setattr(api, "GEOFENCE_PER_SITE_RADIUS_ENABLED", True)
    ...
    residential_id = _create_site(
        "broad unrelated residential",
        latitude=LATITUDE + 0.01,
        geofence_radius_m=15,
        location_type="Residential",
    )
    commercial_id = _create_site(
        "broad nearest commercial",
        geofence_radius_m=15,
        location_type="Commercial",
    )
    db.execute(
    ...
    _inside_employee_id, inside_employee_auth = _create_employee(
        client,
        "broad residential commercial winner inside",
    )
    inside = client.post(
        "/api/timesheet/clock-in",
        headers=inside_employee_auth,
        json={
            **point,
            "latitude": LATITUDE + 0.00009,
        },
    )
    assert inside.status_code == 200, inside.text
    assert inside.json()["entry"]["location"] == (
        f"{PREFIX} broad nearest commercial"
    )
...
def test_clock_boundary_fallback_preserves_gps_less_override(client, monkeypatch):
    _employee_id, employee_auth = _create_employee(client, "gps-less override")
    monkeypatch.setattr(api, "_active_home_base_config", lambda *, cur=None: None)
    monkeypatch.setattr(api, "GEOFENCE_SITE_RESOLUTION_ENABLED", False)
    monkeypatch.setattr(
        api,
    ...
    response = client.post(
        ...
        headers=employee_auth,
        json={
            "location": "Offline dispatch",
            "gpsOverrideReason": "No signal at the work site.",
        },
    )

    assert response.status_code == 200, response.text
    assert response.json()["siteResolution"]["reason"] == "missing_gps"
    assert response.json()["entry"]["location"] == "Offline dispatch"
    assert response.json()["entry"]["clockInGps"] is None
    assert response.json()["entry"]["clockInGpsMeta"] == {
        "override": True,
        "overrideReason": "No signal at the work site.",
        "overrideDetail": "",
        "matchedLocation": "",
        "distanceM": None,
        "withinRadius": None,
...
def test_clock_boundary_refresh_retains_unpinned_customer_metadata(
    client,
    monkeypatch,
):
    employee_id, employee_auth = _create_employee(client, "unpinned customer metadata")
    monkeypatch.setattr(api, "_active_home_base_config", lambda *, cur=None: None)
    monkeypatch.setattr(api, "GEOFENCE_SITE_RESOLUTION_ENABLED", False)
    monkeypatch.setattr(api, "LOCATION_MATCH_RADIUS_M", 50)
    monkeypatch.setattr(
    ...
    suffix = "unpinned customer metadata"
    ...
    response = client.post(
        ...
        json={
            ...
            "longitude": 0,
            "accuracy": 1,
            "gpsOverrideReason": "Supervisor approved the unpinned Site.",
        },
    )

    assert response.status_code == 200, response.text
    assert response.json()["entry"]["location"] == address
    assert response.json()["entry"]["customer"] == f"{PREFIX} Customer {suffix}"
    persisted = db.query_one(
        "SELECT location_id FROM shifts WHERE employee_id = %s ORDER BY id DESC LIMIT 1",
        (employee_id,),
    )
    assert persisted
    assert int(persisted["location_id"]) == site_id
...
def test_final_clock_in_rebuilds_a_renamed_legacy_target_after_reclassification(
    client,
    monkeypatch,
):
    employee_id, employee_auth = _create_employee(
        client,
        "renamed legacy target refresh",
    )
    monkeypatch.setattr(api, "_active_home_base_config", lambda *, cur=None: None)
    monkeypatch.setattr(api, "GEOFENCE_SITE_RESOLUTION_ENABLED", False)
    monkeypatch.setattr(api, "GEOFENCE_PER_SITE_RADIUS_ENABLED", False)
    ...
    original_address = f"{PREFIX} legacy target before refresh"
    current_address = f"{PREFIX} legacy target after refresh"
    site_id = _create_site(
        "renamed legacy target refresh",
    ...
    def rename_and_reclassify_after_provisional_resolution(action, *args, **kwargs):
        nonlocal clock_in_resolutions
        resolution = original_resolver(action, *args, **kwargs)
        if action == "clock-in":
            clock_in_resolutions += 1
            if clock_in_resolutions == 1:
                ...
                db.execute(
                    """
                    UPDATE locations
                    SET address = %s, location_type = 'Residential'
                    WHERE id = %s
                    """,
                    (current_address, site_id),
                )
        return resolution
    ...
    monkeypatch.setattr(
        api,
        "_c3_resolve_site",
        rename_and_reclassify_after_provisional_resolution,
    )
    response = client.post(
        "/api/timesheet/clock-in",
        headers=employee_auth,
    ...
    assert response.status_code == 200, response.text
    assert clock_in_resolutions == 2
    assert response.json()["entry"]["location"] == current_address
    assert response.json()["entry"]["clockInGpsMeta"]["matchedLocation"] == (
        current_address
    )
    assert db.query_one(
        "SELECT location_id, location_label FROM shifts WHERE employee_id = %s",
        (employee_id,),
    ) == {"location_id": site_id, "location_label": current_address}
...
def test_dual_switch_keeps_a_valid_residential_match_near_a_commercial_boundary(
    client,
    monkeypatch,
):
    _employee_id, employee_auth = _create_employee(
        client,
        "dual switch mixed site boundary",
    )
    monkeypatch.setattr(api, "_active_home_base_config", lambda *, cur=None: None)
    monkeypatch.setattr(api, "GEOFENCE_SITE_RESOLUTION_ENABLED", True)
    monkeypatch.setattr(api, "GEOFENCE_PER_SITE_RADIUS_ENABLED", True)
    ...
    residential_id = _create_site(
        "dual switch mixed residential",
        geofence_radius_m=50,
        location_type="Residential",
    )
    commercial_id = _create_site(
        "dual switch mixed commercial",
        geofence_radius_m=15,
        location_type="Commercial",
    )
    db.execute(
    ...
    for site_id in (residential_id, commercial_id):
        row = api._c3_customer_site_rows(
            api.ClockInRequest(locationId=site_id, **point),
            action="clock-in",
            selected_location_id=site_id,
        )[0]
        db.execute(
    ...
    assert response.status_code == 200, response.text
    assert response.json()["entry"]["locationId"] == residential_id
    assert response.json()["siteResolution"]["site"]["locationId"] == residential_id
