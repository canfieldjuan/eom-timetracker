...
def test_a_late_link_adopts_the_contacts_already_mirrored_type(
    client, auth, monkeypatch
):
    """A row linked AFTER a type change must not split the contact's mirror.

    The customer-type route fans a change out to every customer holding the
    contact, but it can only reach rows linked at that moment. This
    reconciliation carries the type Atlas reported when it started, so
    finalizing later would leave two local mirrors of one contact disagreeing.

    Provenance decides: the established row carries an Atlas version, this path
    carries none, so the newcomer adopts the group's value.
    """
    name = _name("Late Link Adopts")
    customer_id = _unlinked_customer(name)

    # An established customer already mirrors this contact at 'commercial',
    # written from a known Atlas version.
    contact_id = fake_atlas_contact_id("late-link-key")
    established = _unlinked_customer(_name("Late Link Established"))
    db.execute(
        "UPDATE customers SET atlas_contact_id = %s, customer_type = %s, "
        "customer_type_source_at = %s WHERE id = %s",
        (contact_id, "commercial", "2026-08-11T12:50:00+00:00", established),
    )

    # Atlas answers this reconciliation with the STALE type it held earlier.
    def _post(url, *, headers=None, json=None, timeout=None):
        from conftest import _FakeAtlasResponse  # type: ignore[attr-defined]
    ...
    assert _stored_type(name) == "commercial", (
        "the late-linked row kept a stale type, splitting the contact's mirror"
    )
    row = db.query_one(
        "SELECT customer_type FROM customers WHERE id = %s", (established,)
    )
